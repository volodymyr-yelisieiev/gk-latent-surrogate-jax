...
retrospective seed-52 test realization, its flux RMSE is 11.4255 versus 11.9889 for persistence, a
4.70\% lower primary point estimate. The paired five-trajectory interval includes zero, so this
experiment does not establish statistical significance, repeatability, or broad superiority.

The run establishes that the complete data,
representation, cache, sequence, diagnostic, and provenance path can be executed with a strict
trajectory-level split. It also shows that a strong persistence reference is
essential for slowly changing latent states: low training loss or a complex temporal architecture
does not by itself demonstrate useful forecasting. Under the present encoder and training budget,
the learned-model point estimate is lower in one test realization, with insufficient independent
replication for a broader claim.
...
The learned model also has lower latent MSE than persistence on the retrospective test. Because diagnostic
...
coordinate scale may matter; matched multi-seed runs are needed to test whether the effect repeats.
