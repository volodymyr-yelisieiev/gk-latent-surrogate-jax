...
selected only as the strongest learned candidate for the pre-declared locked test.
...
The interval includes zero, so no significance or equivalence claim is made.
...
The final test records have no tracked diff (`tracked_diff_sha256` is the SHA-256 of an
